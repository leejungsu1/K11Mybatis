		...
		$.ajax({
			...
			success : function(d){
				if(d.statusCode==0){
					...
					alert("로그인 후 삭제를 시도해 주세요.");
					location.href="./login.do";
				}
				else if(d.statusCode==2){
					/*
					HTML엘리먼트를 숨김처리한다ㅏ. 이때 시간을 부여하면
...
function reflash(){
	/*
	키보드의 F5이나 새로고침 버튼을 누른것과 동일하게 웝브라우저를 새로고침해서
...
function paging(pNum){
	$.ajax({
		...
		success : function(d){
			$('#boardHTML').html('');
			$('#boardHTML').append('<div sytle-"text-align:center;padding-top:50px;">')
