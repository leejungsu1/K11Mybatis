...
public class MybatisController {
	...
	사용할 준비를 한다. @Autowired는 타입만 일치하면 자동으로 주입
	받을수있다.
	 */
	@Autowired
	...
	public String writeAction(Model model, HttpServletRequest req, HttpSession session) {
		...
		//Mybatis 사용 
		sqlSession.getMapper(MybatisDAOImpl.class).write(req.getParameter("name"),
	...
	public String modify(Model model, HttpServletRequest req, HttpSession sessoin) {
		//로그인 확인
		if(sessoin.getAttribute("siteUserInfo")==null) {
			return "redirect:login.do";
		}
		/*
		...
		parameterDTO.setUser_id(
				((MemberVO)sessoin.getAttribute("siteUserInfo")).getId());//사용자 아이디
